...
SIGMA_MIN = deg(-90.0)     # [rad] Minimum bank angle (full left)
SIGMA_MAX = deg(90.0)      # [rad] Maximum bank angle (full right)
...
def solve_collocation(lat_target_in=None, lon_target_in=None):
    ...
    # ===== OBJECTIVE FUNCTION =====
    # We want to MAXIMIZE deployment altitude for safety
    # CasADi minimizes, so we minimize the negative altitude
    h_final = r[-1] - mars.radius  # Altitude at final node [m]
    opti.minimize(-h_final)         # Maximize altitude = minimize (-altitude)
    ...
    # These constraints apply only at the FINAL node (deployment point)
    # They define the parachute deployment window
    ...
    # Parachute can only be deployed in safe Mach and dynamic pressure range
    opti.subject_to(opti.bounded(MACH_MIN, mach_f, MACH_MAX))  # 1.4 ≤ M ≤ 2.2
    opti.subject_to(opti.bounded(Q_MIN, q_f, Q_MAX))           # 300 ≤ q ≤ 800 Pa
    
    # RANGE-TO-GO CONSTRAINT:
    # Must deploy at specific distance from target landing site
    ...
    # Use simple constant bank angle for initial trajectory
    sigma_const = deg(30)  # Moderate positive bank (30 degrees)
    dt_guess = 1.5         # Time step for simulation
    
    # Simulate forward using RK4 to fill state guess
    for k in range(N_SEGMENTS):
        # Integrate one step forward
        state_sim = rk4_step(eom, state_sim, sigma_const, dt_guess)
    ...
    # Initial guess for control: constant moderate bank
    U_guess = np.zeros((1, N_SEGMENTS))
    U_guess[0, :] = deg(30)  # 30 degrees for all segments
    ...
    print(f"Final Rgo: {hist['Rgo'][-1]:.2f} km (target: {RGO_TARGET/1e3:.1f} km)")  # Range constraint
...
def plot_results(sol, hist):
    ...
    # Bank angle, rate, and acceleration (showing bang-bang structure)
    ax6_twin1 = axes[6].twinx()  # Create second y-axis
    ax6_twin2 = axes[6].twinx()  # Create third y-axis
    ax6_twin2.spines['right'].set_position(('outward', 60))  # Offset third axis
    
    # Plot all three quantities
    p1 = axes[6].plot(t, sigma, 'k-', lw=2, label='σ')
    p2 = ax6_twin1.plot(t, np.rad2deg(sigma_dot), 'b-', lw=1.5, alpha=0.7, label='σ̇')
    p3 = ax6_twin2.plot(t, np.rad2deg(sigma_ddot), 'r-', lw=1.5, alpha=0.7, label='σ̈')
    ...
    axes[6].set_ylabel('σ [deg]', color='k')
    ax6_twin1.set_ylabel('σ̇ [deg/s]', color='b')
    ax6_twin2.set_ylabel('σ̈ [deg/s²]', color='r')
    
    axes[6].tick_params(axis='y', labelcolor='k')
    ax6_twin1.tick_params(axis='y', labelcolor='b')
    ax6_twin2.tick_params(axis='y', labelcolor='r')
    
    # Combine legends
    lines = p1 + p2 + p3
    labels = [l.get_label() for l in lines]
    axes[6].legend(lines, labels, fontsize=8, loc='best')
    axes[6].grid(True, alpha=0.3)
    axes[6].set_title('Bank Angle, Rate & Acceleration', fontsize=9)
